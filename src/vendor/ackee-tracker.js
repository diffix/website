// This is a very awkward copy of the raw ackee-tracker
// found here: https://github.com/electerious/ackee-tracker/blob/master/src/scripts/main.js
// The reason it needed to be included is that svelte-ackee is completely broken,
// and the native ackee-tracker version fails to build properly with SvelteKit
// as SvelteKit believes the references to navigator, location, and window are invalid...
// This modified version hard codes those parameters.
...
const source = function() {
	...
	const source = (window.location.search.split(`source=`)[1] || '').split('&')[0]
...
export const attributes = function(detailed = false) {
	...
	const detailedData = {
		siteLanguage: (window.navigator.language || window.navigator.userLanguage).substr(0, 2),
 ...
 */
export const create = function(window, server, opts) {
	...
	if (opts.ignoreLocalhost === true && isLocalhost(window.location.hostname) === true) {
		console.warn('Ackee ignores you because you are on localhost')
		return fakeInstance
	}

	if (isBot(window.navigator.userAgent) === true) {
